document.addEventListener("DOMContentLoaded", function () {
    d3.csv("assets/data/Classification_Cleaned.csv").then(function (data) {
        const nestedData = d3.group(data, d => d.wpm_material_type, d => d.wpm_material_name);
        const treeData = {
            name: "Waste",
            children: Array.from(nestedData).map(([key, values]) => ({
                name: key,
                children: Array.from(values).map(([key, subValues]) => {
                    const total = subValues.reduce((acc, curr) => acc + parseFloat(curr.recovery_rate), 0);
            ...
            node.each(function (d) {
                if (!d.children && !d._children) {
                    ...
                    d3.select(this).append("text")
                        ...
                        .attr("y", 0)
                        .attr("fill", "black")
                        .text(d.data.name + "（recovery: " + (d.data.recoveryRate * 100).toFixed(2) + "%）");
                } else {
                    d3.select(this).append("text")
                        .attr("dy", "-1em")
                        .attr("x", 0)
